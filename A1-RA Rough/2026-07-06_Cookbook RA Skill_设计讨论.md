...
每次 `archive-jira-to-cb` skill 执行完归档后（见该 skill 文件 Step 6），会把这个 ticket 当作 RA skill 的回溯测试样本：反推这个需求本该覆盖哪些分析要点，跟当前 `cookbook-ra.md` 的框架对比，把有价值的发现记在这里。**这里只积累证据，不直接改 skill 文件**——Bonnie 定期回顾这份记录，决定要不要据此更新 RA skill。
