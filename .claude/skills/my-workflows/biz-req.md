...
**`Cookbook-business/` is the dedicated directory for all Cookbook-related business requirements documents.** All biz-req output goes here unless the user specifies a different target directory.
...
- Default output location: `Cookbook-business/`
...
A markdown document placed in `Cookbook-business/` (the dedicated Cookbook business requirements directory), with complete frontmatter, wikilink references, and a source page URL matrix. All content in English.
