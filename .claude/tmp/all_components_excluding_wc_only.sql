...
-- Per-item concept name(s) for output
item_concepts AS (
  SELECT item_number, STRING_AGG(DISTINCT INITCAP(concept_name), ', ' ORDER BY INITCAP(concept_name)) AS brand_names
  FROM `wonder-dw-prod-brd.master_data.recipe_concept_mapping`
  WHERE LOWER(concept_name) IN ('royal greens', 'limesalt', 'yasas', 'hanu poke')
...
-- Extract procedures: only COOK for appliances requiring global config
lb_procedures AS (
  SELECT
    mi.item_number,
    mi.item_name,
  ...
  WHERE JSON_EXTRACT_SCALAR(p, '$.activity') = 'COOK'
    AND JSON_EXTRACT_SCALAR(p, '$.appliance') IN ('TURBO_OVEN', 'FRYER', 'PIZZA_CONVEYOR_OVEN', 'CLAMSHELL')
),
...
SELECT
  ...
  d.activity,
  d.appliance,
  -- Format Global Appliance Config per appliance type
  CASE
    WHEN d.appliance = 'TURBO_OVEN' AND gas.turbo_oven_config IS NOT NULL THEN
      CONCAT(
        JSON_EXTRACT_SCALAR(
    ...
    WHEN d.appliance = 'FRYER' AND gas.fryer_config IS NOT NULL THEN
      CONCAT(JSON_EXTRACT_SCALAR(gas.fryer_config, '$.temperature'), '°F')
    WHEN d.appliance = 'PIZZA_CONVEYOR_OVEN' AND gas.pizza_conveyor_oven_config IS NOT NULL THEN
      CONCAT(
        JSON_EXTRACT_SCALAR(gas.pizza_conveyor_oven_config, '$.name'), ', ',
        JSON_EXTRACT_SCALAR(gas.pizza_conveyor_oven_config, '$.temperature'), '°F, ',
        JSON_EXTRACT_SCALAR(gas.pizza_conveyor_oven_config, '$.top_wind_speed_percentage'), '/',
        JSON_EXTRACT_SCALAR(gas.pizza_conveyor_oven_config, '$.bottom_wind_speed_percentage'), ', ',
        JSON_EXTRACT_SCALAR(gas.pizza_conveyor_oven_config, '$.duration')
      )
    WHEN d.appliance = 'CLAMSHELL' AND gas.clamshell_config IS NOT NULL THEN
      CONCAT(
        'Top: ', JSON_EXTRACT_SCALAR(gas.clamshell_config, '$.top_plate'), '°F, ',
        'Bottom: ', JSON_EXTRACT_SCALAR(gas.clamshell_config, '$.bottom_plate'), '°F, ',
        'Gap: ', JSON_EXTRACT_SCALAR(gas.clamshell_config, '$.gap_setting'), 'mm'
      )
    ELSE NULL
